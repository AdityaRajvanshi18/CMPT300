...
struct sigevent sev;
struct sigaction sigAlarm, sigTerm, sigUser;
...
static void init_sched_queue(int queue_size)
{
	...
	sev.sigev_value.sival_ptr = timer; //ATTENTION REQUIRED MAYBE TIMER SHOULD BE &timer
...
static void *scheduler_run(void *unused){
	...
	timer_settime(timer, 0, &timer_s, NULL);
